...
void VooBandit::Initialize()
{
    ...
    for(int i = 0; i < no_joints; ++i)
    {
        ...
        //std::uniform_real_distribution<double> joint_distribution(-1.0, 1.0);
        ...
        //sample_gaussian_covariance(i, i) = 0.2;
    }
...
void VooBandit::GenerateSamples(boost::shared_ptr<MotionState> current_state, boost::shared_ptr<UserModel> user_model)
{
    ...
    double best_config_reward = UpdateBestConfig(current_state);
    ...
    while(time_remaining)
    {
        ...
        robot_model_lock.unlock();
        double new_sample_reward = new_sample->GetExpectedReward(0, bandit_spec_->rollout_steps, bandit_spec_->gamma, &(current_state->joint_positions));
...
double VooBandit::UpdateBestConfig(boost::shared_ptr<MotionState> start_state)
{
    ...
    for(int i = 0; i < samples.size(); ++i)
    {
        ...
        {
            double sample_reward = (*it)->GetExpectedReward(starting_timestep, bandit_spec_->rollout_steps, bandit_spec_->gamma, &(start_state->joint_positions));
    ...
    RCLCPP_ERROR(LOGGER, "Total samples considered: %.1f", total_counted);
